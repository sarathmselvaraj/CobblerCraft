  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    if(!mine.length){empty.hidden=false;history.innerHTML='';}
      ...
      history.innerHTML=mine.map(b=>`<article class="cc-history-item">
        <div class="min-w-0"><div class="flex flex-wrap items-center gap-2"><h3 class="font-bold">${esc(b.serviceName||'Service')}</h3><span class="cc-booking-status">${esc(b.status||'Requested')}</span></div><p class="muted mt-1 text-sm">Reference ${esc(b.reference||'—')} · Preferred ${esc(formatDate(b.date))}</p>${b.notes?`<p class="muted mt-2 text-sm">${esc(b.notes)}</p>`:''}</div>
