...
(function(){
      ...
      <div class="cc-booking-body">
        ...
        <form data-service-booking-form>
          ...
          <div class="cc-booking-grid">
            ...
            <div class="cc-full"><span class="mb-2 block text-sm font-medium">How will you hand over the footwear?</span><div class="cc-handover-options" role="radiogroup" aria-label="Handover method"><label class="cc-handover-option"><input type="radio" name="fulfillment" value="store" checked><span><strong>Store Handover</strong><small>Bring your footwear directly to the CobblerCraft workshop.</small></span></label><label class="cc-handover-option"><input type="radio" name="fulfillment" value="pickup"><span><strong>Pickup & Delivery</strong><small>We collect the footwear and return it after the repair is completed.</small></span></label></div></div>
            <div class="cc-full cc-pickup-address hidden" data-pickup-address><label class="mb-1.5 block text-sm font-medium" for="cc-book-address">Pickup & delivery address</label><textarea class="input" id="cc-book-address" name="pickupAddress" rows="3" placeholder="House / street, city and postcode"></textarea><p class="muted mt-1.5 text-xs">Pickup availability can be confirmed by the workshop after booking.</p></div>
  ...
  function updateHandoverFields(modal){
    ...
    const pickup=modal.querySelector('[data-pickup-address]');
    const address=modal.querySelector('#cc-book-address');
  ...
  function openBooking(slug){
    ...
    const storeChoice=modal.querySelector('input[name="fulfillment"][value="store"]');
    if(storeChoice)storeChoice.checked=true;
    const savedAddress=[u.address,u.city,u.postcode].filter(Boolean).join(', ');
    const pickupAddress=modal.querySelector('#cc-book-address');
    if(pickupAddress)pickupAddress.value=savedAddress;
    updateHandoverFields(modal);
  ...
  function saveBooking(form){
    ...
    const handoverMethod=data.fulfillment==='pickup'?'Pickup & Delivery':'Store Handover';
    const booking={reference:ref,userId:s.id||'',serviceSlug:activeSlug,serviceName:SERVICES[activeSlug],name:data.name,email:s.email||data.email,phone:data.phone,date:data.date,handoverMethod,pickupAddress:data.fulfillment==='pickup'?(data.pickupAddress||'').trim():'',notes:data.notes||'',createdAt:new Date().toISOString(),status:'Requested'};
  ...
  document.addEventListener('change',e=>{
    if(e.target.matches('[data-service-booking-form] input[name="fulfillment"]')){
      const modal=e.target.closest('[data-booking-modal]');if(modal)updateHandoverFields(modal);
    }
  });
  ...
  document.addEventListener('submit',e=>{
    ...
    const success=form.querySelector('[data-booking-success]');success.textContent=`Booking requested for ${booking.serviceName} via ${booking.handoverMethod}. Reference: ${booking.reference}. View it anytime in My Profile.`;success.classList.add('is-visible');
