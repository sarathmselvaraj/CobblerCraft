...
(function(){
  ...
  function updatePreferenceButtons(){
    ...
    document.querySelectorAll('button[onclick*="ccToggleTheme"]').forEach(btn=>{
      btn.classList.add('cc-pref-toggle','cc-theme-toggle');
      btn.setAttribute('data-theme-state',isDark?'dark':'light');
      btn.setAttribute('aria-label',isDark?'Switch to light mode':'Switch to dark mode');
      btn.innerHTML=isDark
        ? '<span class="cc-pref-icon" aria-hidden="true">☀</span><span class="cc-pref-label">Light</span>'
        : '<span class="cc-pref-icon" aria-hidden="true">◐</span><span class="cc-pref-label">Dark</span>';
    });
    document.querySelectorAll('button[onclick*="ccToggleDir"]').forEach(btn=>{
      btn.classList.add('cc-pref-toggle','cc-dir-toggle');
      btn.setAttribute('data-dir-state',isRtl?'rtl':'ltr');
      btn.setAttribute('aria-label',isRtl?'Switch to LTR layout':'Switch to RTL layout');
      btn.innerHTML='<span class="cc-pref-label">'+(isRtl?'LTR':'RTL')+'</span>';
    });
    const firstPref=document.querySelector('button[onclick*="ccToggleTheme"]');
    const controls=firstPref&&firstPref.parentElement;
    if(controls)controls.classList.add('cc-header-controls');
  }
  ...
  document.addEventListener('DOMContentLoaded',function(){
    ...
    const syncSticky=()=>stickyHeader&&stickyHeader.classList.toggle('cc-header-scrolled',window.scrollY>12);
    syncSticky();window.addEventListener('scroll',syncSticky,{passive:true});
    ...
    if(menu)menu.addEventListener('click',e=>{if(e.target.closest('a'))menu.classList.add('hidden');});
    ...
    // Forms: instant home quotation, newsletter popup and generic demo confirmation.
    const showSiteToast=(message)=>{
      let toast=document.querySelector('[data-site-toast]');
      if(!toast){
        toast=document.createElement('div');
        toast.className='cc-site-toast';
      ...
      window.__ccToastTimer=setTimeout(()=>toast.classList.remove('is-visible'),3600);
    };

    const quoteMap={
      'Sole Replacement':{price:'$58',turnaround:'3–5 days'},
      'Heel Repair':{price:'$24',turnaround:'24 hours'},
      'Leather Polishing':{price:'$19',turnaround:'Same day'},
      'Stitching & Seams':{price:'$32',turnaround:'2–4 days'},
      'Leather Restoration':{price:'$48',turnaround:'2–3 days'},
      'Wax & Creaming':{price:'$24',turnaround:'Same day'},
      'Leather Care':{price:'$29',turnaround:'24 hours'},
      'Sneaker Deep Cleaning':{price:'$35',turnaround:'1–2 days'},
      'Orthopaedic Work':{price:'$95',turnaround:'5–7 days'}
    };
    ...
    document.querySelectorAll('form[data-demo]').forEach(f=>f.addEventListener('submit',e=>{
      ...
      const newsletter=f.querySelector('input[placeholder="Email for repair tips"]');
      if(newsletter){
        const email=newsletter.value.trim();
        showSiteToast('<strong>Joined successfully!</strong><br>Repair tips and care updates will be sent to '+email+'.');
        f.reset();
        return;
      }

      if(f.hasAttribute('data-home-quote')){
        const serviceEl=f.querySelector('#h-s');
        const result=f.querySelector('[data-quote-result]');
        const service=serviceEl?serviceEl.value:'Sole Replacement';
        const quote=quoteMap[service]||{price:'Assessment required',turnaround:'Confirmed after inspection'};
        if(result){
          result.innerHTML='<div class="cc-quote-head"><div><div class="text-xs font-bold uppercase tracking-wider brand">Estimated quotation</div><div class="cc-quote-service mt-1">'+service+'</div></div><div class="cc-quote-price">From '+quote.price+'</div></div><div class="cc-quote-meta">Estimated turnaround: <strong>'+quote.turnaround+'</strong>. Final price is confirmed after a free inspection because material and damage level can affect the total.</div>';
          result.classList.remove('hidden');
          result.scrollIntoView({behavior:'smooth',block:'nearest'});
        }
        showSiteToast('<strong>Quote ready:</strong> '+service+' starts from '+quote.price+'.');
        return;
      }

      const n=f.querySelector('[data-demo-note]');
      if(n){n.textContent='Thanks! Your message has been submitted successfully.';n.classList.remove('hidden');}
    }));
    // dashboard sidebar
